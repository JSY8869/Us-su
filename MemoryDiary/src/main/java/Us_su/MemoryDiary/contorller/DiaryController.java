...
public class DiaryController {
    ...
    public String getDiary(@RequestBody DiaryDto diaryDto) {
        Diary diary = diaryService.getDiary(diaryDto.getIdentifier(), diaryDto.getCreateAt());
        return diary.getContents();
    }

    @PostMapping("/diary/delete")
    public ResponseEntity<Void> deleteDiary(@RequestBody DiaryDto diaryDto) {
        diaryService.deleteDiary(diaryDto.getIdentifier(), diaryDto.getCreateAt());
        return ResponseEntity.ok().build();
    }
}
